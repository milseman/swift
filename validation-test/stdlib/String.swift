...
StringTests.test("CharacterViewReplace") {
  ...
  for s1 in [narrow, wide] {
    for s2 in [narrow, wide] {
      checkRangeReplaceable(
        { String.CharacterView(_fixmeLegacyCore: makeStringCore(s1)) },
        { String.CharacterView(_fixmeLegacyCore: makeStringCore(s2 + s2)[0..<$0]) }
      )
      checkRangeReplaceable(
        { String.CharacterView(_fixmeLegacyCore: makeStringCore(s1)) },
        { Array(String.CharacterView(_fixmeLegacyCore: makeStringCore(s2 + s2)[0..<$0])) }
      )
    }
  }
}
